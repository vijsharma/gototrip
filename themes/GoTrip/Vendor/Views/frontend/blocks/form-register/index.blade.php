...
@endpush
<div class="container">
    <div class="bravo-vendor-form-register layout-pt-lg @if(!empty($layout)) {{ $layout }} @endif">
        <div class="row">
            <div class="col-12 col-lg-5">
                <h1 class="mb-10">{{$title}}</h1>
                <p class="sub-heading mb-30">{{$desc}}</p>
                <form class="form bravo-form-register-vendor" method="post" action="{{route('vendor.register')}}">
                    @csrf

                    <div class="form-group">
                        <input type="text" class="form-control border" name="first_name" autocomplete="off" placeholder="{{__("First Name")}}">
