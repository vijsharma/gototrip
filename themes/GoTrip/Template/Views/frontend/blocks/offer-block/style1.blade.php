        ...
        @if(!empty($list_item))
            <div class="row y-gap-20">
                ...
                @foreach($list_item as $key=>$item)
                    <div class="{{ ($style ?? '' == 'style1') ? 'col-lg-6 col-md-6' : 'col-md-6' }}">

                        <div class="ctaCard -type-1 rounded-4 @if(empty($item['offer_overLay'])) -no-overlay @endif">
                            <div class="ctaCard__image ratio {{ ($style ?? '' == 'style1') ? 'ratio-3:2' : 'ratio-63:55' }}">
                                <img class="img-ratio js-lazy" src="#" data-src="{{ get_file_url($item['background_image'],'full') ?? "" }}" alt="image">
                            </div>

                            <div class="ctaCard__content justify-center d-flex flex-column {{ ($style ?? '' == 'style1') ? 'py-50 px-50' : 'py-70 px-70' }}  lg:py-30 lg:px-30">
								<h4 class="{{ $style ?? '' == 'style1' ? 'text-30 xl:text-24' : 'text-40 lg:text-26' }} text-white">{!! clean($item['title']) !!}</h4>
                                @if(!empty($item['featured_text']))
                                    <div class="text-15 fw-500 lh-15 text-white mb-10">{{ $item['featured_text'] }}</div>
                                @endif 

                                <div class="d-inline-block fit-content mt-30 sm:mt-10">
                                    <a href="{{$item['link_more']}}" class="button px-48 py-10 -blue-1 -min-180 bg-white text-dark-1">{{$item['link_title']}}</a>
                                </div>
                            </div>
                        </div>
