...
@section('content')
    <div class="row y-gap-20 justify-between items-end pb-60 lg:pb-40 md:pb-32">
        <div class="col-auto">
            <h1 class="text-30 lh-14 fw-600"> {{!empty($recovery) ?__('Recovery Events') : __("Manage Events")}}</h1>
        ...
        <div class="col-auto">
            @if(Auth::user()->hasPermission('event_create') && empty($recovery))
                <a href="{{ route("event.vendor.create") }}"  class="button h-50 px-24 -dark-1 bg-blue-1 text-white">
                    {{__("Add Event")}} <div class="icon-arrow-top-right ml-15"></div>
                </a>
            @endif
        </div>
    </div>
