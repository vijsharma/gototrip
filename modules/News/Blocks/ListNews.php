...
class ListNews extends BaseBlock
{
    ...
    public function query($model){
        ...
        $blog_array_id=!empty($model['news_id'])?$model['news_id']:[];
        $tag_array_id=!empty($model['tag_id'])?$model['tag_id']:[];
        if(count($blog_array_id))
            $model_news->whereIn('core_news.id',$blog_array_id);
        if(count($tag_array_id)){
            $model_news->join('core_news_tag', function ($join) use($tag_array_id) {
                    $join->on('core_news_tag.news_id', '=', 'core_news.id')
                        ->WhereIn("tag_id",$tag_array_id);
                });
        }
