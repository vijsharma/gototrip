...
class Tour extends Bookable
{
    ...
    public function saveMeta(\Illuminate\Http\Request $request)
    {
        ...
        if (!empty($arg['robots_meta'])) {
            $arg['robots_meta'] = array_values($arg['robots_meta']);
        }
        if (!empty($arg['notes'])) {
            $arg['notes'] = array_values($arg['notes']);
        }
        if (!empty($arg['tour_links'])) {
            $arg['tour_links'] = array_values($arg['tour_links']);
        }
        if (!empty($arg['tour_mobile_menus'])) {
            $arg['tour_mobile_menus'] = array_values($arg['tour_mobile_menus']);
        }
        if (!empty($arg['tour_feature'])) {
            $arg['tour_feature'] = array_values($arg['tour_feature']);
        }
        if ($request->hasFile('brochure'))
        {
            ...
            //Move Uploaded File
            $filepath=$destinationPath.'/'.$meta->brochure;
            if($file->move($destinationPath,$filename))
            {
                if(file_exists($filepath) && is_file($filepath))
                    unlink($filepath);
            }
            $arg['brochure']=$filename;
        }
        else{
            unset($arg["brochure"]);
        }
    ...
    public function getBookingData()
    {
        $booking_data = [
            ...
            'start_date_html'          => request()->input('start') ? display_date(request()->input('start')) : __('Select date!'),
...
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
        ...
        if (!empty($request['exclude_tour_id'])) {
            $model_Tour->whereNotIn("bravo_tours.id", $request['exclude_tour_id']);
            $model_Tour->orderByRaw('FIELD (bravo_tours.id, ' . implode(', ', $request['exclude_tour_id']) . ') ASC');
        }

        if (!empty($request['attribute_id'])) {
            $attribute_id = is_array($request['attribute_id']) ? $request['attribute_id'] : [$request['attribute_id']];
            $terms = Terms::whereIn('attr_id', $attribute_id)->get();
            if(!empty($terms)){
                $model_Tour
                    ->join('bravo_tour_term', function ($join) {
                        $join->on('bravo_tour_term.tour_id', '=', 'bravo_tours.id');
                    })
                    ->join('bravo_terms', function ($join) use($attribute_id) {
                        $join->on('bravo_terms.id', '=', 'bravo_tour_term.term_id')->whereIn('bravo_terms.id',$attribute_id);
                    });
            }
        }
        ...
        switch ($orderby){
            ...
            default:
                if(!empty($request['order']) and !empty($request['order_by'])){
                    ...
                    $model_Tour->orderBy("bravo_tours.id", "desc");
                }
        }
